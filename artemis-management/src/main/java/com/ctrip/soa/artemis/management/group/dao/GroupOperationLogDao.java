...
public class GroupOperationLogDao {
    ...
    public List<GroupOperationLog> query(final String condition, final List<Object> args) {
        final String sql = "select id, group_id, operation, complete, operator_id, token, extensions, reason, create_time, datachange_lasttime from service_group_operation_log";
        ...
        }, new RowMapper<GroupOperationLog>() {
            @Override
            public GroupOperationLog mapRow(final ResultSet rs, final int arg1) throws SQLException {
                final GroupOperationLog log = new GroupOperationLog();
    ...
    public void insert(final GroupOperationLogModel... logs) {
